...
void main() {
  ...
  test('load skills', () async {
    skillsRepository = _createRepository(skillsJson);

    final skills = await skillsRepository.getSkills();

    expect(skills.length, 18);
    expect(skills[0].index, "acrobatics");
    expect(skills[0].characteristic, Characteristic.DEXTERITY);
  });
}

SkillsRepository _createRepository(String input) {
  return SkillsRepository(() async {
    return input;
  });
}
